...
- **ui-ux-design** (new, 1.0.0): comprehensive UI/UX skill built from a six-agent crawl of Figma's Design basics resource library (~50 articles). SKILL.md carries the parts that have to be read before acting — an accessibility floor that holds in every state, the four states every surface ships (loading, empty, error, success) varied by permission and then broken with real data, the nine interaction states with the behavioral rule each one carries, six hierarchy levers including time, the three IA structures, and the three-tier token model with its no-alias-chaining rule. Five reference files carry depth: `INTERACTION.md` (state CSS, Fitts/Hick/Gestalt, forms), `PROCESS.md` (artifact fidelity, usability testing vs UX validation, scope, pre-launch), `LAYOUT.md` (grids, responsive, eye patterns, landing/pricing/portfolio recipes), `SYSTEMS.md` (tokens, design systems, handoff failure modes), `VISUAL.md` (type and color applied to UI, cultural palette constraints, the style guide as a governed artifact). Scope is deliberately comprehensive and overlaps `typography` and `color-system`, which remain the depth references for type scales and palette construction. `SOURCES.md` lists every crawled URL and records six places the skill diverges from its source — most importantly rejecting Figma's 1.125–1.2 line-height figure for body copy, and correcting its definition of a contrast ratio.

- **ui-ux-design** (2.0.0): two new reference files. `ETHICS.md` covers deceptive patterns — Brignull's 18-type catalogue with the UI shape and honest alternative for each, the four an agent produces by default while faithfully executing an ordinary instruction (preselection, hidden costs, nagging, addictive design), and a behavioral rule for the case the source doesn't address: when a request like "increase signups" is satisfiable by a catalogued pattern, name the pattern, price the honest alternative, and let the user choose — don't silently pick either, and don't refuse the task. Also records where the line is genuinely contested (countdown timers are prohibited in the EU and not uniformly in the US; no formal boundary separates infinite scroll from addictive design). `COMPONENTS.md` carries per-component ARIA, keyboard, and focus contracts for the eight components agents build most — tabs, disclosures, notifications, data tables, menu buttons, toggle buttons, tooltips vs toggletips, and cards — plus the four-step labeling hierarchy that puts `aria-label` last, and the two hard stops from Roselli (never `aria-label` on a link, since text-to-speech ignores it and it fails WCAG 2.5.3; never `aria-description` for content). Notes that Pickering's articles predate native `<dialog>` and `popover`, so his contracts hold but his build-it-yourself framing doesn't.

### Changed

- **ui-ux-design** (1.0.1 → 2.0.0): **breaking** — rebuilt on 20 practitioner and research sources after the v1.0 source base (one crawl of Figma's Resource Library) proved to be a design-tool vendor's marketing content that the skill had already been forced to override in six places. The reversal that forces the major: **disabled-control guidance is now `aria-disabled` plus a guarded handler, not the native `disabled` attribute.** v1.0 told agents to disable a control and pair it with a message explaining why; a natively disabled control leaves the tab order and is exempt from contrast requirements, so the control *and* the message both become unreachable for exactly the users who needed the message (Roselli, updated Jul 2026). The reference CSS had also been styling `:disabled` and `[aria-disabled]` identically, and setting `pointer-events: none` alongside `cursor: wait`, which suppresses the cursor it just set while leaving the element in the tab order. Additions: the response-time ladder (100ms / 400ms Doherty / 1s / 10s, each attributed to what it actually measures, with a note that Nielsen's figures predate mobile networks); NN/g's error-message rubric condensed to six checks, including *preserve what the user typed*, which the skill never mentioned; message severity scaling the container rather than the reverse; Kennedy's three laws of locality; Hobday's measurable geometry (shadow blur = 2× distance, nested radius = outer − gap, button padding 2:1, stacked-surface brightness within 12% dark / 7% light — light interfaces need the *tighter* control); Thomas's build order, which defers identity until after the readability floor; OKLCH authoring with relative-color state derivation, the P3 chroma bump, and `stylelint-gamut` enforcement because browsers clip out-of-gamut colors in a way that shifts hue; Ström-Awn's exponential lightness distribution and Bezold–Brücke hue shift, plus the correction that colorblind verification is a matrix transformation and re-measurement rather than a filter over a screenshot; Atomic Design's tier vocabulary, kept for the templates-to-pages step that exists to expose content variation; Ferreira's Consistent/Opinionated/Flexible component tiers; Friedman's three-layer machine-readable system (spec files, closed token set, audit scripts) — directly load-bearing since an open token set is what lets a generating agent invent values; and accessibility personas asked during the work rather than a WCAG checklist at the end. Two rulings settled ranges rather than adding one: measure is 45–75 targeting 66, with `max-width: 33em` as the implementation since character counts aren't a CSS unit, and line-height is inversely proportional to font size (≥1.5 body, toward 1.0–1.2 for display) — which re-scopes rather than rejects the Figma figure v1.0 had thrown out, since 1.125–1.2 is correct for headlines. `LAYOUT.md` also splits the baseline-grid claim: pixel-perfect alignment is unachievable on the web, but vertical spacing as multiples of the base leading unit is not optional. `SOURCES.md` rebuilt with a currency table for the three claims that look stale and aren't — most importantly that **4.5:1 / 3:1 remains the floor**, because APCA was removed from the WCAG 3 draft in July 2023 and WCAG 3 contrast is undetermined with finalization expected 2030+, so "modernize this to APCA" is the plausible-sounding wrong edit. Records which rules rest on paywalled reads and which findings were deliberately excluded (Refactoring UI rules sourced only from third-party Medium summaries; Growth.Design's uncited +14% and 280% metrics; Better Web Type, whose own site returned 403 for the third consecutive research attempt). One reported conflict was checked and dismissed: NN/g's Heuristic 5 does not recommend disabling submit buttons — it reads "eliminate error-prone conditions, or check for them and present users with a confirmation option," which is error prevention, not control disabling. Description 893 → 948 chars, adding ARIA, accessibility, and conversion triggers. Motion and animation remain the largest gap; none of the 20 sources covers it well, and `SOURCES.md` now says so explicitly rather than leaving the silence ambiguous.
...
- **ui-ux-design** (1.0.0 → 1.0.1): graded A (94/100) by `rate-skill`; two findings applied. Cut the payload `INTERACTION.md` duplicated from SKILL.md — transition timing, focus-ring dimensions, disabled-needs-explanation, loading-disables-control, and contrast-in-every-state each appeared in both files, one near-verbatim. The reference table now opens with a pointer and carries only what the SKILL.md summary leaves out. Sharpened the description's routing clause to name the query shape that belongs to `typography` and `color-system` ("only about a type scale, a line-height, or a palette and its contrast") rather than gesturing at depth, since comprehensive scope makes those the two neighbors most likely to collide. Description 842 → 893 chars.
