...
- **rate-skill** (4.0.0 → 5.0.0) and **generate-skill** (4.0.0 → 5.0.0): the meta-pair is now anchored on the platform prompting guides ([Prompting Claude Opus 5](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-opus-5), [Prompting best practices](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/claude-prompting-best-practices)) in addition to the agentskills.io pages it already tracked. Grades under the 4.0.0 rubric are not comparable.
  ...
  - **§5 Examples now grades diversity, not just presence.** Fewer than 3 examples, or 3+ that all exercise the same situation, caps the category at 80 (official guidance is 3–5, relevant and diverse). The `<Good>`/`<Bad>` deduction is narrowed to the *labels* — `<example>` / `<examples>` as structural delimiters are officially recommended and score fine, a distinction the 4.0.0 rule blurred. **§5 also now states that examples living in `references/EXAMPLES.md` behind a pointer count in full** — a gap that would have marked down every skill built the way `local-first-app` is built, since the rule never said where examples had to live and a grader reading only SKILL.md would score a pointer as an omission.
  ...
  - **Both bodies were compressed on the `local-first-app` model** — headings that carry the claim, rationale folded into the rule as an em-dash aside instead of a following sentence, a `## Navigation` load-when table, and all ✅/❌ payload moved behind a one-line pointer. `generate-skill`'s frontmatter detail moved to `references/FRONTMATTER.md` and its source list to `references/SOURCES.md` (mirroring rate-skill); `references/PATTERNS.md` dropped its duplicate section-spec table and its placeholder-heavy skeletons, keeping the per-type shape guidance that isn't already in SKILL.md. Net effect: every new rule above landed **without** growing either body — rate-skill 212 → 187 lines (~3,490 → ~3,530 tokens), generate-skill 237 → 191 lines (~4,160 → ~3,620 tokens), PATTERNS.md 316 → 207 lines.
