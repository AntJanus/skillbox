...
- **local-first-app**: prose rewritten on the `i-have-adhd` rules — lead with the rule, cut section preambles and "why this matters" narration longer than the rule it precedes, one idea per bullet, gotchas compressed toward symptom → cause → fix. Combined with dropping cross-file duplication (the CRUD prose ARCHITECTURE.md restated from SKILL.md, the shell and theming points UI.md restated from CHROME.md, the navbar-overflow rule CHROME.md stated twice), the skill totals **~23,100 tokens across seven files, down from ~32,800** — SKILL.md 6,801 → 5,345 body tokens, ARCHITECTURE.md 8,553 → 5,831, CHROME.md 6,166 → 5,118, UI.md 3,491 → 2,628. Every code sample and every documented failure mode is retained; the cuts are connective tissue. SKILL.md remains ~7% over the ~5,000-token guidance, down from ~30% over.
