...
- **evolve-skills** (experimental, v0.1.0 → **v0.2.0**, minor): convention-compliance pass. Collapsed the multiline `description: |` block scalar (silent discovery break, anthropics/skills #9817) to a single-line directive third-person description with negative scoping (`Do NOT use to grade one skill / scaffold a new skill`). Added `Task` to `allowed-tools` — Phase 4 dispatches parallel patch-proposal agents and could not run without it. Renamed `reference/` → `references/` (plural, canonical) and updated all links. Extracted the full EVOLUTION_REPORT.md template to `references/report-format.md`, trimming the body from 310 → ~262 lines (under the 300 aim). Also fixed the `experimental/README.md` lifecycle template (`reference/` → `references/`) and its stale "None yet" current-skills line.
