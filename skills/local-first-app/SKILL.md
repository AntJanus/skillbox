...
**The rule is import purity, not a directory count.** `src/<domain>/` may not import React, Next, or the DB — that single constraint is what makes the core testable, reusable and packageable. The five directories below are how that rule is expressed on disk.
...
- **A route smoke gate — assert each route returns a *page*.** A 200 is not proof it rendered: a server component passing a function across the client boundary (`<Button component={Link}>`) makes React drop that element and serialise the error into the flight payload, with the status still 200 — so the button an empty state depends on is simply not there. Assert the body contains what the route is for and carries no boundary-error string. Typecheck green + tests green + production build green + a 500 in the browser with no component name in the stack is the single most-reported failure in this blueprint's history. **Discover routes, don't list them** — walk `app/` for `page.tsx`; a hand-maintained list drifts the moment someone adds a page. Recurse into `(groups)` leaving the URL prefix unchanged (a group is organisational; its children are real routes — skipping the subtree found *zero* routes in an app keeping all 67 pages under `(app)`/`(auth)`), skip `[dynamic]` and anything nested under it and `api`, and resolve a `page.tsx` directly inside a top-level group to `/`. **Assert the discovery found something**, or the gate passes by discovering nothing. With no server up, print a visible SKIPPED notice — a gate that quietly passes when it never ran is worse than no gate.
...
- **A hydration gate.** Server/client HTML mismatch is the most frequent runtime error class in this family and is normally found by a user pasting a React error dump from the browser; nothing in a standard suite catches it. *Cheapest form:* load each route in a headless browser, click the interactive controls, and assert the page rendered content **and** that no `error` or `unhandledrejection` fired.
- **A legibility gate.** The readability floor has four parts and **only two are visible in source**: size and weight are theme tokens, but line-height resolves against the *computed* font-size and contrast depends on the *painted* background — the nearest non-transparent ancestor, composited through alpha. So an app can override every size token exactly as instructed and still ship a wordmark at 1.00:1, text colored identically to what's behind it. *The gate:* load each route headless, **assert it found text nodes at all** (a page that rendered nothing otherwise scores a clean zero issues), then walk every one, resolve the effective background, compute the ratio, and fail under 4.5:1 (3:1 for large text). One scoping rule keeps it honest — apply the 1.5 leading floor to **running text only**, since single-line button labels and display headings legitimately set tighter. The same walk measures line length (see below).
...
**Prove each gate bites before trusting it** — break the thing it guards and watch it fail. A backup routine that writes an empty file must fail every restore test; a server returning 500 must fail the route checks. A gate that cannot fail is not a gate.
...
- **Legibility (recurring failure):** set the floor at the theme/globals level — body ≥16px, weight ≥400, contrast ≥4.5:1, line-height ≥1.5. Cards/tables need a surface background distinct from the canvas; flat same-color containers read as illegible. **Setting the floor in the theme does not achieve it** — apps routinely override the size scale exactly as instructed and still ship text under 4.5:1. Treat the theme as necessary and insufficient, and ship the **legibility gate** above; the floor is only real once something fails when it's breached.
- **Line length is part of legibility.** Cap running text near 65–85 characters. A prose column that inherits the full width of a dashboard shell lands past 100 and reads badly even at a perfect contrast ratio.
- **Anything whose whole job is to look different must be *measurably* different.** A theme picker whose swatches are `#0d1117`, `#161b22`, `#18181b`, `#1f1714`, `#0b1015` renders as five identical black squares — every pair between 1.00:1 and 1.10:1. The theme system underneath can be perfectly correct while the control for choosing it is useless. Swatches, status dots and category chips need a real perceptual gap, and it's worth asserting rather than eyeballing.
- **Explainability** *(when the app computes non-obvious numbers)*: show each formula once (KaTeX) with values plugged in + a JavaScript-form toggle; collapsible "underlying math" under dashboard stats; an in-app `/docs` area with Mermaid diagrams. A pure CRUD tracker skips formulas. **Ship the `/docs` concept area only when the app has a concept a new user would get wrong** — a term the UI uses in a specific sense ("missing" vs "deleted", "stale", "root"), or a rule the user must model to trust the output. A music library has no such concept, and mandating a page there produces a stub nobody reads.
...
- **`BASE_SCHEMA` is migration v1 — one construction path.** Every database, fresh or existing, reaches its current shape by replaying the chain from its own `user_version`. There is no separate path for a new install, so there is no second definition of the schema to keep in sync. A fresh-DB fast path (run `BASE_SCHEMA`, jump `user_version` to the max) forces you to keep two schemas in manual agreement forever, and the failure is asymmetric: dev machines replay and stay green while **every new install and every packaged binary** gets the other schema. Replaying N migrations against a local SQLite file costs milliseconds — the fast path optimises something that was never slow.
- **Migrations:** track `PRAGMA user_version`, run on boot. Run each `up()` + its `user_version` bump in *one* transaction (no half-applied state); keep each migration idempotent (`CREATE IF NOT EXISTS`, guarded `ALTER ... ADD COLUMN`).
- **Snapshot before any pending migration — this is a requirement, not a nicety.** Local-first means there is no server copy, so a bad migration is unrecoverable user data. `VACUUM INTO` a `backups/` directory that is a **sibling** of the data dir (so `rm -rf data/` spares it); skip it for a fresh database, where there's nothing to protect. Three details the obvious version gets wrong: **stamp to milliseconds *and* break ties with a counter** (`…-2.sqlite`) — two programmatic calls really do land in one millisecond, and letting `VACUUM INTO` throw on a collision means either migrating with no backup or refusing to start; **prune by timestamp, not filename** — a lexical sort puts `v10-<old>` before `v2-<new>`, so an oldest-first prune deletes the *newer* snapshot once you pass v9, and "keep 20" means 20 backups, not 20 per schema version; and **keep snapshot and rotation as two calls** — a failed snapshot is fatal, a failed rotation must not be, since the snapshot already exists and dying in cleanup blocks the migration it just protected. See references/ARCHITECTURE.md.
