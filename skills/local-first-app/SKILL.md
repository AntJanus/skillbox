...
## The five gates a green test suite does not give you

Unit tests over the pure core pass while the app is broken in the browser. These five gates close the gaps that a typecheck, a full suite, and a clean production build all miss.

- **A route smoke gate.** Something must actually request every route and assert it returns a page. Typecheck green + tests green + production build green + a 500 in the browser with no component name in the stack is the single most-reported failure in this blueprint's history — a server component rendering a client-only compound component is invisible to every other check. *Cheapest form:* start the server, `curl` each route, fail on any non-2xx.
- **A restore gate, not a backup gate.** Asserting that a backup file appeared proves nothing. A backup routine that calls a `.backup()` method which doesn't exist on `node:sqlite` throws on first real use — the safety net missing exactly when it's needed, while a test that only checks for a file stays green. **Write the backup, open the copy, read a row back out.** Also stamp filenames to sub-second precision: a pre-migration backup racing a scheduled one collides, and `VACUUM INTO` throws if the target exists.
- **A hydration gate.** Server/client HTML mismatch is the most frequent runtime error class in this family and is normally found by a user pasting a React error dump from the browser; nothing in a standard suite catches it. *Cheapest form:* load each route in a headless browser, click the interactive controls, and fail on any `error` or `unhandledrejection`.
- **A legibility gate.** The readability floor has four parts and **only two are visible in source**: size and weight are theme tokens, but line-height resolves against the *computed* font-size and contrast depends on the *painted* background — the nearest non-transparent ancestor, composited through alpha. So an app can override every size token exactly as instructed and still ship a wordmark at 1.00:1, text colored identically to what's behind it. *The gate:* load each route headless, walk every text node, resolve the effective background, compute the ratio, and fail under 4.5:1 (3:1 for large text). One scoping rule keeps it honest — apply the 1.5 leading floor to **running text only**, since single-line button labels and display headings legitimately set tighter. The same walk measures line length (see below).

**Fixture realism is the multiplier on all five.** A five-row fixture passes while a predicate missing from the SQL makes a 5,000-row library report zero results, because `LIMIT` truncated before the pure filter ran. At least one test must exceed the page size, and filesystem features must be tested against real files on disk.
