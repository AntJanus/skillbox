...
- **A schema-drift gate.** There are two paths to a schema — replay the migrations, or build from `BASE_SCHEMA` — and dev machines only ever take the first while packaged binaries only ever take the second. Add a table to one and not the other and **every new install breaks while every dev machine stays green**. Build a DB both ways and diff them.
