...
*An app with **no persisted entities** skips this machinery — a pure core plus a form, no `src/db`, migrations, loaders or screen shells.*
...
- **Theming is two axes, both server-side:** the named theme *and* the light/dark scheme persist in the settings table and render into `<html data-theme>` + `forceColorScheme`. No pre-paint script, no flash, no hydration mismatch, and the header toggle renders one icon rather than hiding one of two.
