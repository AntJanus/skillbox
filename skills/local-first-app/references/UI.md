...
Applies when the app derives non-obvious numbers (projection or estimate tools). A pure CRUD tracker with no computed figures skips the formula surfaces below. When there *are* computed numbers, the product is trusted only if the user can audit them:
...
- **Ship an in-app `/docs` area only when the app has a concept a new user would get wrong** — a term the UI uses in a specific sense ("missing" vs "deleted", "stale", "root"), or a rule the user must model to trust the output. Where it earns its place, explain that concept with **Mermaid** diagrams (lazy client-only `import('mermaid')`, themed to the active scheme, surfacing parse errors instead of failing silently) and keep README + project docs in sync with it. A domain with no such concept — a plain media library — produces a stub nobody reads; skip it.
...
- **Two of the four floor values are not visible in source at all — reading the theme can never verify them.** Size and weight are tokens you can grep. Line-height resolves against the *computed* font-size, and contrast depends on the *painted* background — the nearest non-transparent ancestor, composited through alpha — neither of which exists until a page renders. So an app can override every token exactly as instructed and still ship a wordmark at 1.00:1, text colored identically to what sits behind it. Setting the floor in the theme is necessary and insufficient; ship the **legibility gate** (SKILL.md): load each route headless, walk every text node, resolve the effective background, and fail under 4.5:1 (3:1 for large text). Scope the 1.5 leading floor to **running text only** — single-line button labels and display headings legitimately set tighter.
- **Line length is part of legibility.** Cap running text near 65–85 characters; a prose column that inherits the full width of the dashboard shell lands past 100 and reads badly at any contrast ratio. The same text-node walk measures it.
- **Anything whose whole job is to look different must be *measurably* different.** Theme swatches, status dots and category chips need a real perceptual gap. A picker whose swatches sit within 1.00:1–1.10:1 of each other renders as identical squares while the theme system underneath is perfectly correct and server-persisted — the control for choosing a theme is useless while nothing about the theming is wrong. Assert the gap rather than eyeballing it.
