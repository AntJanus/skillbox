...
- **Versioned migrations** tracked by `PRAGMA user_version`, run on boot. **`BASE_SCHEMA` is migration v1**, so there is exactly one way to construct a schema: every database — fresh or existing — runs each migration with `version > user_version`, in order. A fresh DB simply starts at `user_version = 0` and replays the whole chain. Don't add a fast path that runs `BASE_SCHEMA` and jumps `user_version` to the max: it makes `BASE_SCHEMA` and the chain two definitions that must be kept in manual agreement, and the drift is invisible to whoever introduces it (dev machines replay and stay green) while breaking **every new install and every packaged binary**. Replaying N migrations against a local file costs milliseconds. Each migration is idempotent (`CREATE IF NOT EXISTS`, guarded `ALTER TABLE ... ADD COLUMN`) and runs **inside one transaction together with its `user_version` bump**, so a multi-statement migration can't half-apply and desync the counter:
